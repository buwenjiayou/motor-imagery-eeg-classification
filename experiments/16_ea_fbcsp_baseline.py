"""
Euclidean Alignment + FBCSP-LDA cross-session experiment.

Protocol:
  Per subject, train sessions 1-4 and test session 5.

Important setting:
  EA uses unlabeled train/test EEG samples to estimate domain covariance. Test
  labels are never used. Report this as unsupervised target alignment.
"""
...
def euclidean_align(x: np.ndarray) -> np.ndarray:
    whitening = inv_sqrtm_spd(mean_covariance(x))
...
class CSP:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> "CSP":
        ...
        c0 = mean_covariance(x[y == classes[0]], reg=self.reg)
        c1 = mean_covariance(x[y == classes[1]], reg=self.reg)
...
def run_subject(subject: str, n_components: int) -> ResultRow:
    ...
    model = FBCSPLDA(DEFAULT_BANDS, n_components=n_components)
...
def main() -> None:
    ...
    rows = []
    for subject in tqdm(args.subjects, desc="EA+FBCSP"):
    ...
    table_path = RESULTS_TABLES / "ea_fbcsp_full_all_subjects.csv"
    summary_csv_path = RESULTS_TABLES / "ea_fbcsp_full_summary.csv"
    summary_json_path = RESULTS_LOGS / "ea_fbcsp_full_summary.json"
